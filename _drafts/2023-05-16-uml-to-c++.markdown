...
Ein UML-Klassendiagramm stellt die verschiedenen Klassen in einem System, ihre Attribute, Operationen und ihre Beziehungen zueinader dar. Bei richtiger Anwendung vermittelt UML präzise, wie Klassen Diagramme in Code umgewandelt werden können. Das Kernelement, und auch das was am meisten Schwierigkeiten macht, ist die  korrekte Interpretation der verschiedenen Beziehungen der Klassen zueinander. 

Im folgenden stelle ich wichtigsten Beziehungstypen in UML Klassen-Diagrammen dar. Jede Beziehung wird kurz erklärt, danach folgt ein Alltagsbeispiel um sie mit physikalisch greifbaren Objekten zu veranschaulichen. Letzlich wird ein Abschnitt mit einem C++ Codebeispiel abgeschlossen.

Die Intention dieses Artikels ist es, auch "Code-lastigen" Menschen, wie z.B. mir, klar zu machen wie  ein UML-Klassendiagran in Code realisiert wird. Er wendet sich  an Entwickler die mit UML schon Erfahrung haben. Allerdings glaube ich das auch das Anfänger davon profitieren können die die ersten Gehversuche mit UML unternehmen.

# Assoziation 

Bei einer Assoziation handelt sich nicht um eine "kennt ein"-Beziehung, keines der Objekte ist Teil oder Mitglied eines anderen. Sie ist die schwächste der Besitz anzeigenden Beziehung.
...
Die Aggregation ist eine spezielle Form der Assoziation, die eine "hat-ein" Beziehung darstellt. Ich habe ein Objekt, das ich mir von jemand anderem geliehen habe. Auch wenn das Objekt stirbt, kann ich weiterleben.
...
**Alltagsbeispiel:** Mensch und Herz - Ein Mensch hat ein Herz. Der Mensch kann nicht ohne das Herz existieren. Wenn das Herz stirbt, stirbt auch der Mensch. Hier ist die Komposition von der Klasse "Herz" zur Klasse "Mensch".
...
Generalisierung ist ein anderer Begriff für Vererbung. Es handelt sich um eine "ist-ein"-Beziehungen. Wir weisen damit einen Objekt ein Oberbegriff (Kategorie) zu. 
...
Abhängigkeiten repräsentieren eine "benutzt-ein"-Beziehung zwischen zwei Klassen, bei der eine Änderung in einer Klasse Änderungen in der abhängigen Klasse erfordern kann. 
...
Eine Realisierung ist eine Beziehung zwischen zwei Klassen, bei der eine Klasse das Verhalten, das durch eine andere Klasse oder in vielen Fällen durch eine Schnittstelle definiert ist, verspricht oder "realisiert". Man könnte sagen, dass es sich um eine "erfüllt-die"-Beziehung handelt.
...
**Alltagsbeispiel:**  Es fällt schwer ein Alltagsbeispiel zu finden da sich dieses Konzept der realen Welt weitestgehend entzieht. Ganz grob könnte man behaupten das eine Klasse eine Schnittstelle "Laufbar" implementieren, die eine Methode "laufen" definiert. Diese Klasse könnte ein "Hund", ein "Mensch" oder ein "Roboter" sein - alles, was "laufen" kann.
